...
namespace Chess {
    ...
    void do_move(Move m, Board& b, Boardhistory& bh, Piece pawn_promotion){
        ...
        /*impossible castling
        int pawn_dir = (b.get(m.to).piece_color == Color::white) ? -1 : 1;
        int from = m.to;
        switch(b.get(m.to).piece){
                    case Piece::pawn:
                        if(b.get(from+pawn_dir*11).piece == Piece::king_castle)
                            b.get(from+pawn_dir*11).piece = Piece::king_in_check;
                        if(b.get(from+pawn_dir*9).piece == Piece::king_castle)
                            b.get(from+pawn_dir*9).piece = Piece::king_in_check;
                        if(b.get(from+pawn_dir*10).piece == Piece::king_castle)
                            something needed
                        break;
                    case Piece::knight:
